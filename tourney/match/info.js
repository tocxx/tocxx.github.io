...
async function compileMaps() {
  clearMapCols();
  await addMap(picksAndBans.map1);
  await addMap(picksAndBans.map2);
  await addMap(picksAndBans.map3);
  await addMap(picksAndBans.map4);
  await addMap(picksAndBans.map5);
}
...
function pickedby(id) {
  ...
  if (picksAndBans.map4 == id)
    return `<h5>Picked by ${picksAndBans.p2.name}</h5>`;
  if (picksAndBans.map5 == id) return `<h5>Tiebreaker</h5>`;
